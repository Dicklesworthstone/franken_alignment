//! Bounded, strict JSON input parsing for operator-owned `xtask` inputs.
//!
//! This module deliberately has no serializer and does not define a product
//! canonical encoding. Its job is to preserve the lexical number spelling and
//! reject ambiguous or malformed operator input before admission code inspects
//! it.
...
pub fn parse(input: &[u8], limits: Limits) -> Result<Json, Error> {
    if input.len() > limits.max_bytes {
        return Err(Error::at("", limits.max_bytes, ErrorKind::SizeLimit));
    }
