# Bounded generation through the hosted residual monitor

## Status and relationship to existing generation

Development implementation; compilation, formatting, Clippy and execution qualification are pending. No existing execution receipt qualifies this source. This adds no dependencies, runtime, tokenizer, model import format, actor command, effect capability or persistent event format.

This is the residual-monitor path used by `MonitoredSampledDecoder` and the numerical owner inside `OversightBroker`. It does not replace or change the separate learned-KV generator described in [MONITORED_GENERATION.md](MONITORED_GENERATION.md). That generator stages and audits K/V before numerical publication; this existing residual-monitor owner computes and commits a sampled draw before its residual review can hold. The new drivers preserve their own underlying owner's semantics rather than mixing those distinct contracts.
