...
The reference workspace contains exactly two dependency-free local packages. `xtask` rejects any lockfile outside this exact inventory. This intentionally crude guard is sound for this closed initial inventory, not a general parser or future package-policy engine. Expanding it requires implementing the complete target-feature/source admission gate first. `release-check` refuses release because production qualification is absent.
