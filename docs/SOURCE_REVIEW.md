...
The factored `franken-kernel 0.4.9` has a smaller static surface, but still requires complete serde/derive closure review and cannot replace the Asupersync runtime. This was source inspection, not a resolved per-target Cargo inventory, donor build, runtime artifact audit or admission. FA-053 retains the blocker: the current native pin has no process feature boundary excluding its FFI, and the external-admission implementation, individually reviewed package decisions and executed per-target closure evidence remain missing. The inspected build script can invoke host Git for revision metadata; this alone proves neither a C build nor a second executor. No complete transitive or runtime closure was executed.
...
## 2026-09-08: declared view and trace reference seams

`crates/fa-reference/src/evidence_view.rs` belongs to L2, serving the existing Evidence-view commitments concordance entry (FI-A06/FI-A14, plan §17.7). Private manifests retain caller-declared provenance and the entire actual helper input; constructor consistency is distinct from authenticity. Independent raw desynchronization and accepted alternate-view tests cover both representations. Source windows declare proper-subset truncation, never downstream provider behavior.

`crates/fa-reference/src/trace_independence.rs` belongs to L7, serving Structural sharing and lawful trace quotient (FI-I13/FI-I15, plan §§11.7–11.8). Registered distinct-operator pairs are assumptions. All dependency classes share one whole-domain namespace: writes, RNG advancement and resource consumption conflict with every access in the same domain, while read/read alone remains compatible. Unknown dependencies preserve order; no dependency parser infers semantic reads from explanation text. Retained input mappings distinguish evidence-source and omission namespaces.

The source review found a real cross-category collision gap and the root repaired it. The frozen complete gate, actual planted redaction/RNG/negative-domain failures, fixed release costs and exact-binary allocation profiles are linked by `artifacts/execution/2026-09-08-view-trace-receipt.json`. Public 25-category controls and bounded schedule enumeration defend the implemented seam; they do not prove general commutation, DPOR coverage, capture authenticity or deployment correctness.

## 2026-09-08: exact-value and absent-key witness seam

`crates/fa-reference/src/witness.rs` belongs to L2 and serves the existing Epistemic MVCC judgment witnesses concordance addition (FI-A13/FI-A17, plan §§7.6–7.9). Private captured witnesses retain exact value bytes and versions with their query role, while absence retains a domain projection and exact caller-trusted terminal marker. Revision, control cut, semantics and domain identity are checked before reuse; positive exact reads remain meaningful without global absence closure. No scope or authority is inferred from missing evidence.

Independent review added a semantic-only public negative and an unchanged absence control before insertion. Two actual one-guard mutations fail at the changed-byte and insertion assertions. The source and public limit tests bound snapshots to 256 entries of at most 8 KiB and captures to 64 requests. The cost review corrected a successfully-captured-byte label on a refusal and reran its executable evidence. `artifacts/execution/2026-09-08-witness-receipt.json` binds the full gate, raw counterexamples, descriptive cases and exact-binary profiles. Adapter authentication is a declared input assumption; no control-store adapter, generic database substitution or production authority is implemented.

### 2026-09-08 observation algebra and inventory loader review

The thirteen operators in plan §14.7 now execute pure admission/transfer laws in `observation_algebra.rs`, below physical planning. Captured observations belong to L1; authorized/prefix/verified/decoded/exact/refined/closed-scope evidence to L2; discovery, probes, independent helper judgments, joins and emitted judgments to L3. The single Knowledge wrapper retains epistemic status, while seven claim classes and four scoped proof predicates keep declarations distinct from proved facts. Caller-registered law identities and supplied decoded/helper bytes do not prove the corresponding computation ran. The existing founding-concordance engineering addition “Observation-plan algebra and cost accounting” serves FI-A13/FI-A17; no plan meaning or agent verb changed.

Independent review repaired lost captured bytes on cancellation, conflated uncertainty/origin admission, and premature cardinality checks on unions. Failed transforms borrow active records; context obligations are proposals admitted only on success, not a durable effect ledger. Witness occurrences retain logical byte charges even when their immutable allocations are shared. The [499-test receipt](../artifacts/execution/2026-09-08-algebra-inventory-receipt.json) records exact controls, three planted failures, bounded costs and allocation attribution; it does not qualify capture, decoding, inference, scheduling, permits or persistence.

The L5 `perimeter_inventory.rs` loader performs bounded regular-file reads and strict validation of eleven effect classes, family/profile generations, trust paths, four declared trust domains, credential dispositions and nonclaims. Family-wide direct-credential refusal applies when any route declares brokered effects; observation/cooperative residuals remain explicit. Paired route metadata prevents accidentally selecting a route without its declaration. This is declaration validation, not enforcement or credential discovery. File replacement between metadata and open, authenticity, real endpoint behavior and deployed perimeter completeness remain outside the evidence; FA-002 stays in progress.

### 2026-09-08 range and phantom witness review

The L2 witness validator now retains exact ordered membership or an empty predicate over bounded finite half-open ranges. Capture preflights aggregate member count/value bytes before cloning. Reuse checks monotonic snapshot/control cuts, semantic/domain/projection identity, exact authenticated terminal closure and the actual current ordered members. Public exact-scan comparisons include inserted/deleted members, unchanged-version byte changes, closure loss and exclusive endpoints. Logical scan counts follow entries actually visited, including early refusals; errors expose no partial counter.

A conservative-summary completeness declaration cannot establish exact absence or membership, even with the same store bytes and a separately recorded exact frontier. The positive control supplies the exact closing marker instead. This implements a declared evidence-kind boundary; it neither computes nor validates the soundness of approximate summaries, which remain FA-060 work. Adapter origin/completeness remain caller-trusted assumptions. The existing Epistemic MVCC engineering addition serves FI-A13/FI-A17 without new plan semantics, layer objects or agent verbs. [The 511-test receipt](../artifacts/execution/2026-09-08-range-witness-receipt.json) retains actual guard-mutation failures and scoped costs; no database, runtime, cache authorization or production effect is qualified.
