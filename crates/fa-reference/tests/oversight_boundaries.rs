//! Deadline, provider-change, policy-rotation and containment controls for the
//! observed delivery facade. The endpoint and clock remain reference inputs.
...
use fa_reference::action::consequence::oversight::{
    CommitteeContract, CommitteeInput, HelperContract, MAX_OBSERVED_ROUNDS,
...
use fa_reference::evidence_view::{EvidenceViewManifest, ProjectionBinding, ViewSpec};
use fa_reference::full_input::{ActualHelperInput, InputPart, InputProfileBinding, Omission, OmissionKind, PartKind};
...
fn config() -> ControllerConfig {
    ControllerConfig {
        scope: spec(0).scope, total: 100, max_attempts: 8,
...
fn contracts() -> CommitteeContract {
    CommitteeContract::new(BTreeMap::from([("helper".to_owned(), HelperContract::new(
        InputProfileBinding { profile_id: "helper-v1".to_owned(), generation: 1, model_space: "m".to_owned(),
            model_epoch: 1, tokenizer: "t".to_owned(), tokenizer_epoch: 1, policy_epoch: 0,
            input_contract: "framed-action-v1".to_owned() }, 9, b"Review".to_vec(),
    ).unwrap())])).unwrap()
}
...
fn manifest(action: &FrozenAction, contract: &HelperContract, omissions: Vec<Omission>, projection_id: u64) -> EvidenceViewManifest {
    ...
    EvidenceViewManifest::capture(&input, ViewSpec {
        projection: ProjectionBinding { tenant: 1, projection_id, allowed_originals: Vec::new() },
        views: Vec::new(), exact_submitted: bytes,
    }).unwrap()
}
fn inputs(action: &FrozenAction, contracts: &CommitteeContract) -> CommitteeInput {
    CommitteeInput::capture(action, contracts, BTreeMap::from([("helper".to_owned(),
        manifest(action, &contracts.members()["helper"], Vec::new(), 9),
    )])).unwrap()
}
fn prepare(broker: &mut OversightBroker, contracts: &CommitteeContract, id: u64) -> (FrozenAction, CommitteeInput) {
    ...
    let inputs = inputs(&proposal.action, contracts);
...
fn complete(mut session: ObservedSession, tick: u64) -> ObservedReview {
    let value = session.commitment("helper", Verdict::Allow, b"salt").unwrap();
    session.commit("helper", value, ElapsedTick(tick)).unwrap();
    session.open_reveals(ElapsedTick(tick)).unwrap();
    session.reveal("helper", Verdict::Allow, b"salt", ElapsedTick(tick)).unwrap();
    session.finish(ElapsedTick(tick)).unwrap()
}
fn approve(broker: &mut OversightBroker, id: u64, round: u64, input: &CommitteeInput) {
    let tick = broker.inspect().ledger.elapsed.unwrap().0;
...
fn late_reveal_is_missing_even_when_its_commitment_was_timely() {
    ...
    let value = session.commitment("helper", Verdict::Allow, b"salt").unwrap();
    session.commit("helper", value, ElapsedTick(1)).unwrap();
    ...
    assert_eq!(review.missing(), &["helper".to_owned()]);
...
fn an_entirely_silent_round_can_close_at_the_deadline() {
    ...
    let review = session.finish(ElapsedTick(20)).unwrap();
    assert_eq!(review.decision().consequence, Consequence::HoldEffect);
    assert_eq!(review.missing().len(), 1);
}
...
fn invalid_reveal_does_not_count_as_phase_completion() {
    ...
    assert_eq!(session.reveal("helper", Verdict::Allow, b"wrong", ElapsedTick(2)), Err(Error::Binding));
    assert_eq!(session.finish(ElapsedTick(2)).unwrap_err(), Error::Incomplete);
    session.reveal("helper", Verdict::Allow, b"salt", ElapsedTick(3)).unwrap();
    let review = session.finish(ElapsedTick(3)).unwrap();
...
fn a_future_completed_review_cannot_advance_the_controller_clock() {
    ...
    assert_eq!(broker.apply_review(complete(session, 2), Some(&input), &snapshot()), Err(Error::Stale));
...
fn discarded_round_ids_are_not_reused_with_a_new_context() {
    ...
    let session = broker.begin_review(1, 11, [1; 32], window(), &snapshot()).unwrap();
    drop(session);
    assert_eq!(broker.begin_review(1, 11, [2; 32], window(), &snapshot()).unwrap_err(), Error::Duplicate);
    approve(&mut broker, 1, 12, &input);
}
...
fn helper_model_replacement_cannot_hide_behind_identical_submitted_bytes() {
    ...
    let mut profile = contracts.members()["helper"].profile_at(0);
    profile.model_epoch += 1;
    let changed_contracts = CommitteeContract::new(BTreeMap::from([("helper".to_owned(),
        HelperContract::new(profile, 9, b"Review".to_vec()).unwrap(),
    )])).unwrap();
    let changed = inputs(&action, &changed_contracts);
    assert_eq!(original.views()["helper"].input().submitted(), changed.views()["helper"].input().submitted());
    assert_eq!(broker.record_inputs(1, 1, changed), Err(Error::Binding));
    assert_eq!(broker.input_revision(1), Ok(1));
    approve(&mut broker, 1, 11, &original);
}

#[test]
fn declared_memory_gap_and_wrong_projection_refuse_but_closed_absence_is_usable() {
    let action = FrozenAction::freeze(spec(0)).unwrap();
    let contracts = contracts();
    let helper = &contracts.members()["helper"];
    for status in [OmissionKind::Gapped, OmissionKind::Unsupported, OmissionKind::Redacted, OmissionKind::ClosedAbsent] {
        let view = manifest(&action, helper, vec![Omission { kind: PartKind::Memory, ordinal: 0, status }], 9);
        let result = CommitteeInput::capture(&action, &contracts, BTreeMap::from([("helper".to_owned(), view)]));
        if status == OmissionKind::ClosedAbsent { assert!(result.is_ok()); }
        else { assert_eq!(result, Err(Error::Incomplete)); }
    }
...
fn policy_rotation_invalidates_old_reviews_but_allows_new_epoch_views() {
    ...
    let (_, old_input) = prepare(&mut broker, &contracts, 1);
    let session = broker.begin_review(1, 11, [1; 32], window(), &snapshot()).unwrap();
    let review = complete(session, 1);
    broker.replace_policy(0, 0, policy(2)).unwrap();
    assert_eq!(broker.apply_review(review, Some(&old_input), &snapshot()), Err(Error::Stale));
    let (action, input) = prepare(&mut broker, &contracts, 2);
    assert_eq!(input.views()["helper"].input().profile().policy_epoch, 1);
    approve(&mut broker, 2, 12, &input);
    let permit = broker.authorize(2, Some(&input), &snapshot()).unwrap();
...
fn reset_requires_new_view_and_review_without_restoring_an_old_permit() {
    ...
    let (old_action, old_input) = prepare(&mut broker, &contracts, 1);
    approve(&mut broker, 1, 11, &old_input);
    let old_permit = broker.authorize(1, Some(&old_input), &snapshot()).unwrap();
    let before = broker.inspect();
...
fn round_capacity_cannot_obstruct_cancellation_of_existing_reservations() {
    let (mut broker, _, contracts) = fixture();
    let (_, input) = prepare(&mut broker, &contracts, 1);
    approve(&mut broker, 1, 1, &input);
    let _permit = broker.authorize(1, Some(&input), &snapshot()).unwrap();
    for round in 2..=MAX_OBSERVED_ROUNDS {
        broker.begin_review(1, round as u64, [1; 32], window(), &snapshot()).unwrap();
    }
...
fn invalid_roster_does_not_consume_the_endpoint_attachment() {
    let contracts = contracts();
    let wrong = CommitteeContract::new(BTreeMap::from([("other".to_owned(), contracts.members()["helper"].clone())])).unwrap();
    let mut endpoint = PublicationEndpoint::new(spec(0).target.unwrap(), Vec::new(), 200, 8).unwrap();
    assert_eq!(OversightBroker::new(config(), &mut endpoint, wrong).unwrap_err(), Error::Binding);
    OversightBroker::new(config(), &mut endpoint, contracts).unwrap();
}
...
fn an_exact_policy_violation_still_denies_without_waiting_for_helper_inputs() {
    ...
    violation.values.insert(7, vec![8]);
    let proposal = broker.propose(1, spec(0), &violation).unwrap();
    assert_eq!(proposal.state, ActionState::Denied);
    assert_eq!(broker.input_revision(1), Ok(0));
    assert_eq!(broker.inspect().ledger.available, 100);
    assert_eq!(broker.authorize(1, None, &violation).unwrap_err(), Error::Incomplete);
}
